...
class ProductStockMapperTest {
    ...
    void selectByProductId_在庫を取得できる() {
        ProductStockEntity stock = productStockMapper.selectByProductId(productId);
    ...
    void decreaseQuantity_在庫を減らせる() {
        productStockMapper.decreaseQuantity(productId, 3);

        assertThat(productStockMapper.selectByProductId(productId).getQuantity()).isEqualTo(7);
    }

    @Test
    void lockByProductId_ロックして取得できる() {
        ProductStockEntity stock = productStockMapper.lockByProductId(productId);
